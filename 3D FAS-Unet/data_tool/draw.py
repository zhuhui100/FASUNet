...
def save_3D_result(image_3D, pred_3D, gt_3D, img_name, save_dir, color_dict):
    ...
    for k in range(pred_3D.shape[2]):          
        ...
        image = image_3D[:, :, k, 0] / image_3D[:, :, k, 0].max() *255    # 灰度图片
        pred =  pred_3D[:, :, k]              
        gt = gt_3D[:, :, k]                
        img_name_2d = img_name + "_" + str(k)
        
        ###-------------------------------
        img_edge = add_edge(image, gt, pred)[...,::-1]  
        ...
        cv2.imwrite(img_dir + img_name_2d + ".png", image)
        cv2.imwrite(img_dir + img_name_2d + "_bound.png", img_edge)
